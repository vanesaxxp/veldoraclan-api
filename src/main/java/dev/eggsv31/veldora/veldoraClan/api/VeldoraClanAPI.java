// src/main/java/dev/eggsv31/veldora/veldoraClan/api/VeldoraClanAPI.java
...
/**
 * Merkezi API: hem GUI/metotları hem de IClanDataStorage altındaki tüm işlemleri sağlar.
 */
public interface VeldoraClanAPI {
    // --- Temel API ---
    IClanDataStorage getClanDataStorage();                       // Veriye direkt erişim

    // --- IClanDataStorage metodları (kolay erişim için) ---
    JavaPlugin getPlugin();                                      // plugin instance
    File getClanFolder();                                        // klan klasörü
    DataStorage getDataStorage();                                // low-level storage
    YamlLangManager getLangManager();                            // dil yöneticisi
    ClanStatsManager getClanStatsManager();                      // istatistik yönetimi
